...
def stream_video(kod):
    ...
    if not range_header:
        ...
        response = Response(generate_chunked(video_path, 0, min(first_chunk, file_size)), 206, mimetype="video/mp4")
...
def download_film(kod):
    ...
    return send_file(video_path, as_attachment=True, download_name=f"{film_nomi}.mp4", mimetype='video/mp4')
...
def download_shorts(id):
    ...
    return send_file(video_path, as_attachment=True, download_name=f"{sarlavha}.mp4", mimetype='video/mp4')
...
if __name__ == '__main__':
    ...
    print(f"🔐 Admin panel: /admin (parol: {ADMIN_PASSWORD})")
